...
BotDNAWindow_Class::BotDNAWindow_Class(const Robot *bot, FXComposite *parent) :
    ...
    _bot(bot)
{
    ...
    CurrBotDebugControls.SetBot(_bot);
    ...
    //Setup DNA
    {
        ...
        DNATextBox->setText(bot->getDnaText(&Engine.parser).c_str(),
                            strlen(bot->getDnaText(&Engine.parser).c_str()));
...
{
    DNATextBox->setText(_bot->getDnaText(&Engine.parser).c_str(),
                        (int)strlen(_bot->getDnaText(&Engine.parser).c_str()));
