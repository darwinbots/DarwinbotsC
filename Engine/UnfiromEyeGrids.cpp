...
{
    assert(me != NULL && "Attempting to insert a non existant bot into the eye grid");
...
{
    assert(bot != NULL && "Attempting to remove a non existant bot from the eye grids");
...
{
    assert(bot != NULL && "Attempting to move a non existant bot in the eye grids");
    if(!bot->View)
        return;
...
{
    assert(me != NULL && "Non existant bot queing the eye grid");
